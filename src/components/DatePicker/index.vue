      ...
      <div class="ub" id="wrapper-parent">
        ...
        >
          ...
          >
            <li></li>
            <li></li>
            <li v-for="item in years" :key="item">{{item}}</li>
            <li></li>
            <li></li>
          </ul>
        </div>
        ...
        >
          ...
          >
            <li></li>
            <li></li>
            <li v-for="item in months" :key="item">{{item}}</li>
        ...
        >
          ...
          >
            <li></li>
            <li></li>
            <li v-for="item in dates" :key="item">{{item}}</li>
...
currentDateDetail.month =
  currentDateDetail.month < 10
    ? "0" + currentDateDetail.month
    : currentDateDetail.month;
currentDateDetail.date =
  currentDateDetail.date < 10
    ? "0" + currentDateDetail.date
    : currentDateDetail.date;
export default {
  name: "v-date-picker",
  props: {
  ...
  data() {
    ...
    let dates = this.getRangeArray(
      1,
      this.getMonthMaxDay(seletedDateDetail.year, seletedDateDetail.month),
      true
    );
    let containerOptions = {
      ...
      dateContainer:
        this.type.toString() === "1" ? this.getContainerOptions() : {}
    };
    ...
    childShow(currentVal, oldVal) {
      if (currentVal) {
        if (this.childShow) {
          ...
          let yearDefaultLen =
            this.containerOptions.yearContainer.stepLen *
            this.years.indexOf(beforeSelected.year);
          let monthDefaultLen =
            this.containerOptions.monthContainer.stepLen *
            this.months.indexOf(beforeSelected.month);
          this.setScrollTo(
            1,
            this.containerOptions.yearContainer,
            yearDefaultLen
          );
          this.setScrollTo(
            2,
            this.containerOptions.monthContainer,
            monthDefaultLen
          );
          if (this.type.toString() === "1") {
            let dateDefaultLen =
              this.containerOptions.dateContainer.stepLen *
              this.dates.indexOf(beforeSelected.date);
            this.setScrollTo(
              3,
              this.containerOptions.dateContainer,
              dateDefaultLen
            );
          }
        }
  ...
  methods: {
    ...
    touchMoveItem(e, containerOption, columnsType) {
      if (containerOption) {
        ...
        // 最少移动10px
        if (
          timestamp - containerOption.endTime > 300 &&
          Math.abs(diffPageY) < 10
        ) {
          return;
        }
        ...
        if (movePageY > 0) {
          ...
          Math.abs(movePageY) > Math.abs(containerOption.scrollHeight)
        ) {
          movePageY =
            Math.abs(containerOption.scrollHeight) - Math.abs(movePageY);
    ...
    touchEndItem(e, containerOption, columnsType) {
      if (containerOption) {
        ...
        if (
          ...
          Math.abs(containerOption.offsetTop) >
            Math.abs(containerOption.scrollHeight)
        ) {
          //上边缘&下边缘
          ...
          let transitionStyle =
            "all " + moveTime + "ms " + "cubic-bezier(0.1, 0.57, 0.1, 1)";
          this.setTransition(columnsType, true, transitionStyle);
        } else {
        ...
        if (containerOption.options.step && containerOption.stepLen > 0) {
          ...
          let moveY =
            h >= halfHeight
              ? nowEndY - containerOption.stepLen + h
              : nowEndY + h;
        ...
        let transformStyle =
          "translate(0, " + containerOption.offsetTop + "px)";
        this.setTransition(columnsType, false, transformStyle);
        if (this.type.toString() === "1") {
          ...
          if (parseInt(this.date) > monthMaxDay) {
            ...
            let moveLen =
              (monthMaxDay - 1) * this.containerOptions.dateContainer.stepLen;
            this.setScrollTo(
              3,
              this.containerOptions.dateContainer,
              moveLen,
    ...
    getYearHeight(containerOptions) {
      ...
      containerOptions.yearContainer.scrollerHeight = this.$refs[
        this.yearId
      ].clientHeight;
      ...
      containerOptions.yearContainer.scrollHeight =
        yearNodeHeight - containerOptions.yearContainer.scrollerHeight;
      containerOptions.yearContainer.transition = this.yearTransition;
      containerOptions.yearContainer.transform = this.yearTransform;
      containerOptions.yearContainer.stepLen =
        childNodes.length > 0 ? childNodes[0].clientHeight : 0;
      return containerOptions;
    },
    getMonthHeight(containerOptions) {
      ...
      containerOptions.monthContainer.scrollerHeight = this.$refs[
        this.monthId
      ].clientHeight;
      ...
      containerOptions.monthContainer.scrollHeight =
        monthNodeHeight - containerOptions.monthContainer.scrollerHeight;
      ...
      containerOptions.monthContainer.stepLen =
        childNodes.length > 0 ? childNodes[0].clientHeight : 0;
      return containerOptions;
    },
    getDateHeight(containerOptions) {
      ...
      containerOptions.dateContainer.scrollerHeight = this.$refs[
        this.dateId
      ].clientHeight;
      ...
      containerOptions.dateContainer.scrollHeight =
        dateNodeHeight - containerOptions.dateContainer.scrollerHeight;
      containerOptions.dateContainer.transition = this.yearTransition;
      containerOptions.dateContainer.transform = this.yearTransform;
      containerOptions.dateContainer.stepLen =
        childNodes.length > 0 ? childNodes[0].clientHeight : 0;
      return containerOptions;
    },
